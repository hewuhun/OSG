...
void CSulGeomQuad::setTexture( osg::Image* pImage, GLint internalFormat, sigma::uint32 unit )
{
	...
	osg::ref_ptr<osg::Texture2D> pTex = new osg::Texture2D;
	...
	getOrCreateStateSet()->setTextureAttributeAndModes( unit, pTex.get(), osg::StateAttribute::ON );
}

void CSulGeomQuad::setTexture( osg::Texture* pTex, sigma::uint32 unit, const std::string& uniformName )
{
	createUV();
	m_mapTex[unit] = pTex;
...
osg::Texture2D* CSulGeomQuad::setTexture( const std::string& file, sigma::uint32 unit )
{
	createUV();
