...
{
	return 0;//(sigma::int64)_strtoi64( c_str(), 0, 10 );
}
...
osg::Vec3 CSulString::asVec3() const
{
	...
	{
		float r, g, b;
        //sscanf_s( c_str(), "%f,%f,%f,%f", &r, &g, &b ); /// w00024 由sscanf变为sscanf_s
...
osg::Vec4 CSulString::asVec4() const
{
	...
	{
		float r, g, b, a;
        //sscanf_s( c_str(), "%f,%f,%f,%f", &r, &g, &b, &a );
...
osg::Vec4s CSulString::asVec4s() const
{
	...
	{
		int r, g, b, a;
        //sscanf_s( c_str(), "%i,%i,%i,%i", &r, &g, &b, &a );
        sscanf( c_str(), "%f,%f,%f,%f", &r, &g, &b, &a);
...
CSulString CSulString::urlGetEncode() const
{
	...
	for(int i=0; i<max; i++)
	{
		...
		{
			escaped.append("%");
			escaped.append( char2hex(c_str()[i]) );
		}
	}
