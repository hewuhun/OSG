...
namespace FeKit
{
	...
	namespace
	{
		...
		struct ManipOldTerrainCallback : public TerrainCallback
		{
			ManipOldTerrainCallback(PreEarthManipulator* manip) : _manip(manip) { }
			...
			void onTileAdded( const TileKey& key, osg::Node* tile, TerrainCallbackContext& context )
			{
				...
				{
					_manip->recalculateCenter(tile);
				}
			}

			osg::observer_ptr<PreEarthManipulator> _manip;
		};
	}


#define HASMODKEY( W, V ) (( W & V ) == V )
#define LC "[EarthOldManip] "
	...
	PreEarthManipulator::Action::Action( ActionType type, const ActionOptions& options ) :
	_type( type ),
		_options( options )
	{ 
		init();
	...
	PreEarthManipulator::Action::Action( ActionType type ) :
	_type( type )
	{
		init();
	}
	...
	void
		PreEarthManipulator::Action::init()
	{
		_dir =
	...
	PreEarthManipulator::Action::Action( const Action& rhs ) :
	_type( rhs._type ),
		_dir( rhs._dir ),
	...
	bool 
		PreEarthManipulator::Action::getBoolOption( int option, bool defaultValue ) const
	{
	...
	int 
		PreEarthManipulator::Action::getIntOption( int option, int defaultValue ) const
	{
		for(ActionOptions::const_iterator i = _options.begin(); i != _options.end(); i++ ) {
			if ( i->option() == option )
				return i->intValue();
	...
	double 
		PreEarthManipulator::Action::getDoubleOption( int option, double defaultValue ) const
	{
		for(ActionOptions::const_iterator i = _options.begin(); i != _options.end(); i++ ) {
			if ( i->option() == option )
	...
	PreEarthManipulator::Action PreEarthManipulator::NullAction( PreEarthManipulator::ACTION_NULL );


	PreEarthManipulator::Settings::Settings() :
	osg::Referenced                 (),
		Revisioned                      (),
	...
	PreEarthManipulator::Settings::Settings( const PreEarthManipulator::Settings& rhs ) :
	osg::Referenced( rhs ),
		Revisioned     ( rhs ),
	...
	void
		PreEarthManipulator::Settings::expandSpec( const InputSpec& input, InputSpecs& output ) const
	{
	...
	void
		PreEarthManipulator::Settings::bind( const InputSpec& spec, const Action& action )
	{
		InputSpecs specs;
	...
	void PreEarthManipulator::Settings::bindMouse(ActionType actionType,
	...
	void PreEarthManipulator::Settings::bindMouseClick(ActionType action,
		int button_mask, int modkey_mask,
		const ActionOptions& options)
	{
		bind(
	...
	void PreEarthManipulator::Settings::bindMouseDoubleClick(ActionType action,
		int button_mask, int modkey_mask,
		const ActionOptions& options)
	{
		bind(
	...
	void PreEarthManipulator::Settings::bindKey(ActionType action,
	...
	void PreEarthManipulator::Settings::bindScroll(ActionType action, int scrolling_motion,
	...
	void PreEarthManipulator::Settings::bindPinch(ActionType action, const ActionOptions& options)
	{
		bind(
			InputSpec( PreEarthManipulator::EVENT_MULTI_PINCH, 0, 0 ),
			Action( action, options ) );
	}

	void PreEarthManipulator::Settings::bindTwist(ActionType action, const ActionOptions& options)
	{
		bind(
			InputSpec( PreEarthManipulator::EVENT_MULTI_TWIST, 0, 0 ),
			Action( action, options ) );
	}

	void PreEarthManipulator::Settings::bindMultiDrag(ActionType action, const ActionOptions& options)
	{
		bind(
			InputSpec( PreEarthManipulator::EVENT_MULTI_DRAG, 0, 0 ),
			Action( action, options ) );
	}

	const PreEarthManipulator::Action&
		PreEarthManipulator::Settings::getAction(int event_type, int input_mask, int modkey_mask) const
	{
	...
	void
		PreEarthManipulator::Settings::setMinMaxPitch( double min_pitch, double max_pitch )
	{   
		_min_pitch = osg::clampBetween( min_pitch, -179.9, 179.9 );
		_max_pitch = osg::clampBetween( max_pitch, min_pitch, 179.9 );
		dirty();
	...
	void
		PreEarthManipulator::Settings::setMaxOffset(double max_x_offset, double max_y_offset)
	{
	...
	void
		PreEarthManipulator::Settings::setMinMaxDistance( double min_distance, double max_distance)
	{
	...
	void
		PreEarthManipulator::Settings::setArcViewpointTransitions( bool value )
	{
	...
	void
		PreEarthManipulator::Settings::setAutoViewpointDurationEnabled( bool value )
	{
	...
	void
		PreEarthManipulator::Settings::setAutoViewpointDurationLimits( double minSeconds, double maxSeconds )
	{
	...
	void
		PreEarthManipulator::Settings::setCameraProjection(const PreEarthManipulator::CameraProjection& value)
	{
		_camProjType = value;
		dirty();
	}
	...
	void
		PreEarthManipulator::Settings::setCameraFrustumOffsets( const osg::Vec2s& value )
	{
	...
	PreEarthManipulator::PreEarthManipulator() :
	...
	PreEarthManipulator::PreEarthManipulator( const PreEarthManipulator& rhs ) :
	osgGA::CameraManipulator( rhs ),
		_last_action            ( ACTION_NULL ),
		_last_event             ( EVENT_MOUSE_DOUBLE_CLICK ),
		_time_s_last_event      (0.0),
		_frame_count            ( 0 ),
		_scrollDuration         (rhs._scrollDuration),
		_continuousPow          (rhs._continuousPow),
	...
	void
		PreEarthManipulator::configureDefaultSettings()
	{
	...
	void
		PreEarthManipulator::applySettings( Settings* settings )
	{
		if ( settings )
		{
			_settings = settings;
		...
		if ( new_pitch != old_pitch )
		{
			...
			setViewpoint( Viewpoint("",vp.focalPoint().get().x(),vp.focalPoint().get().y(),vp.focalPoint().get().z(), vp.getHeading(), new_pitch, vp.getRange(),vp.focalPoint().get().getSRS()));
		}
	}

	PreEarthManipulator::Settings*
		PreEarthManipulator::getSettings() const
	{
		return _settings.get();
	}

	void
		PreEarthManipulator::reinitialize()
	{
		_distance = 1.0;
		_offset_x = 0.0;
		_offset_y = 0.0;
	...
	bool
		PreEarthManipulator::established()
	{
		...
		if ( needToReestablish )
		{
			...
			// find a map node.
			_mapNode = MapNode::findMapNode( safeNode.get(), _findNodeTraversalMask );
			if ( _mapNode.valid() && !_settings->getDisableCollisionAvoidance() )
			{            
				_terrainCallback = new ManipOldTerrainCallback( this );
			...
			if ( _csn.valid() )
			{
				...
				if ( !_homeViewpoint.isSet() )
				{
					...
					{
						setHomeViewpoint( 
							Viewpoint("",-90,0,_csn->getEllipsoidModel()->getRadiusEquator()*3.0, 0, -89,
							0 ,getSRS()) );
					}
					else
					{
						setHomeViewpoint( Viewpoint("",
							safeNode->getBound().center().x(),
							safeNode->getBound().center().y(),
							safeNode->getBound().center().z(),
							0, -89.9, 
							0,
							getSRS()) 
							);
	...
	bool
		PreEarthManipulator::createLocalCoordFrame( const osg::Vec3d& worldPos, osg::CoordinateFrame& out_frame ) const
	{
	...
	void
		PreEarthManipulator::setCenter( const osg::Vec3d& worldPos )
	{
		_center = worldPos;
	...
	void
		PreEarthManipulator::setNode(osg::Node* node)
	{
	...
	osg::Node*
		PreEarthManipulator::getNode()
	{
		return _node.get();
	}

	const osgEarth::SpatialReference*
		PreEarthManipulator::getSRS() const
	{
		osg::ref_ptr<osg::Node> safeNode = _node.get();

		if ( !_cached_srs.valid() && !_srs_lookup_failed && safeNode.valid() )
		{
			PreEarthManipulator* nonconst_this = const_cast<PreEarthManipulator*>(this);
	...
	osg::Matrixd
		PreEarthManipulator::getRotation(const osg::Vec3d& point) const
	{
		...
		//He Quan
		//2015.5.18
		double offset = 10000;//1e-6;

		return osg::Matrixd::lookAt( point - (lookVector * offset), point, up);
	}

	void
		PreEarthManipulator::setViewpoint( const Viewpoint& vp, double duration_s )
	{
		if ( !established() ) 
		{
		...
		else if ( duration_s > 0.0 )
		{
			...
			_delta_focal_point = vpFocalPoint - svp;
		...
		else
		{
			...
			if ( getSRS() )
			{
				...
				osg::ref_ptr<const SpatialReference> vp_srs = vp.focalPoint().get().getSRS()? vp.focalPoint().get().getSRS() :
	...
	Viewpoint
		PreEarthManipulator::getViewpoint() const
	{
	...
	void
		PreEarthManipulator::setTetherNode( osg::Node* node )
	{
	...
	osg::Node*
		PreEarthManipulator::getTetherNode() const
	{
	...
	bool
		PreEarthManipulator::intersect(const osg::Vec3d& start, const osg::Vec3d& end, osg::Vec3d& intersection) const
	{
		osg::ref_ptr<osg::Node> safeNode = _node.get();
		if ( safeNode.valid() )
		{
	...
	void
		PreEarthManipulator::home(double unused)
	{
		handleAction( ACTION_HOME, 0, 0, 0 );
	}

	void
		PreEarthManipulator::home(const osgGA::GUIEventAdapter& ,osgGA::GUIActionAdapter& us)
	{
		home( 0.0 );
		us.requestRedraw();
	}
	...
	void
		PreEarthManipulator::computeHomePosition()
	{    
		if( getNode() )
		{
	...
	void
		PreEarthManipulator::init(const osgGA::GUIEventAdapter&, osgGA::GUIActionAdapter& )
	{
		flushMouseEventStack();
	}
	...
	void
		PreEarthManipulator::getUsage(osg::ApplicationUsage& usage) const
	{
	}

	void
		PreEarthManipulator::resetMouse( osgGA::GUIActionAdapter& aa, bool flushEventStack )
	{
		if (flushEventStack)
			flushMouseEventStack();
	...
	void
		PreEarthManipulator::updateCamera( osg::Camera* eventCamera )
	{
	...
	bool
		PreEarthManipulator::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
	{
		bool handled = false;
		...
		if ( ea.getEventType() == osgGA::GUIEventAdapter::FRAME )
		{
			...
			// start HGT: 添加实现osgEarth定位功能 w00024 
			...
			{
				bool stillRunning = serviceTask(ea,aa);
				if ( stillRunning ) 
				{
		...
		if ( !handled )
		{
			...
			case osgGA::GUIEventAdapter::PUSH:
				...
				mousePushEvent(ea,aa, _settings->getAction(osgGA::GUIEventAdapter::DRAG,ea.getButtonMask(),0L));
			...
			case osgGA::GUIEventAdapter::SCROLL:
				...
				if ( handleScrollAction( action, _scrollDuration ) )
					aa.requestRedraw();
				handled = true;
				break;
			default: break;
	...
	void
		PreEarthManipulator::postUpdate()
	{
		updateTether();
	}

	void
		PreEarthManipulator::updateTether()
	{
	...
	bool
		PreEarthManipulator::serviceTask(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
	{
		if ( _task.valid() && _task->_type != TASK_NONE )
		{
	...
	bool
		PreEarthManipulator::isMouseMoving()
	{
	...
	bool
		PreEarthManipulator::isMouseClick( const osgGA::GUIEventAdapter* mouse_up_event ) const
	{
	...
	void
		PreEarthManipulator::flushMouseEventStack()
	{
		_ga_t1 = NULL;
		_ga_t0 = NULL;
		//_touchPointQueue.clear();
	...
	void
		PreEarthManipulator::addMouseEvent(const osgGA::GUIEventAdapter& ea)
	{
		_ga_t1 = _ga_t0;
		_ga_t0 = &ea;
		//_touchPointQueue.clear();
	...
	void
		PreEarthManipulator::addTouchEvents(const osgGA::GUIEventAdapter& ea)
	{
		_ga_t1 = _ga_t0;
		_ga_t0 = &ea;
	...
	bool
		PreEarthManipulator::parseTouchEvents( TouchEvents& output )
	{
	...
	void PreEarthManipulator::setByMatrix(const osg::Matrixd& matrix)
	{
	...
	osg::Matrixd PreEarthManipulator::getMatrix() const
	{
	...
	osg::Matrixd PreEarthManipulator::getInverseMatrix() const
	{
	...
	void PreEarthManipulator::setByLookAt(const osg::Vec3d& eye,const osg::Vec3d& center,const osg::Vec3d& up)
	{
		osg::ref_ptr<osg::Node> safeNode = _node.get();
	...
	void PreEarthManipulator::recalculateCenter( const osg::CoordinateFrame& frame, osg::Node* tile )
	{
		osg::ref_ptr<osg::Node> safeNode = _node.get();
		if ( safeNode.valid() )
		{
	...
	void PreEarthManipulator::pan( double dx, double dy )
	{
		if (!_tether_node.valid())
		{
	...
	void PreEarthManipulator::rotate( double dx, double dy )
	{
	...
	void PreEarthManipulator::zoom( double dx, double dy )
	{    
	...
	bool PreEarthManipulator::screenToWorld(float x, float y, osg::View* theView, osg::Vec3d& out_coords ) const
	{
		if(!_mapNode.valid())
			return false;
		...
		return mapNode->getTerrain()->getWorldCoordsUnderMouse(view, x, y, out_coords); // 直接进行转换
	}

	void PreEarthManipulator::setDistance( double distance )
	{
		_distance = osg::clampBetween( distance, _settings->getMinDistance(), _settings->getMaxDistance() );
	}

	void PreEarthManipulator::dumpActionInfo( const PreEarthManipulator::Action& action, osg::NotifySeverity level ) const
	{
	...
	void PreEarthManipulator::handleMovementAction( const ActionType& type, double dx, double dy, osg::View* view )
	{
		switch( type )
		{
		case ACTION_PAN:
	...
	bool PreEarthManipulator::handlePointAction( const Action& action, float mx, float my, osg::View* view )
	{
	...
	void PreEarthManipulator::handleContinuousAction( const Action& action, osg::View* view )
	{
	...
	void PreEarthManipulator::applyOptionsToDeltas( const Action& action, double& dx, double& dy )
	{
	...
	bool PreEarthManipulator::handleMouseAction( const Action& action, osg::View* view )
	{
		...
		if ( _continuous )
		{
			_continuous_dx += dx * _continuousPow;
			_continuous_dy += dy * _continuousPow;
		}
		else
		{
	...
	bool PreEarthManipulator::handleMouseClickAction( const Action& action )
	{
		//TODO.
		return false;
	}

	bool PreEarthManipulator::handleKeyboardAction( const Action& action, double duration )
	{
		double dx = 0, dy = 0;
	...
	bool PreEarthManipulator::handleScrollAction( const Action& action, double duration )
	{
	...
	bool PreEarthManipulator::handleAction( const Action& action, double dx, double dy, double duration )
	{
		bool handled = true;
	...
	void PreEarthManipulator::recalculateRoll()
	{
		osg::Matrixd rotation_matrix;
	...
	void PreEarthManipulator::getLocalEulerAngles( double* out_azim, double* out_pitch ) const
	{
	...
	void
		PreEarthManipulator::setHomeViewpoint( const Viewpoint& vp, double duration_s )
	{
		_homeViewpoint = vp;
	...
	void
		PreEarthManipulator::drag(double dx, double dy, osg::View* theView)
	{
		using namespace osg;
	...
	{
		if(fabs(input) > 2*osg::PI)	input = fmod(input,2*osg::PI);
