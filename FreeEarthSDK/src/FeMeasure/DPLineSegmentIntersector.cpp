...
namespace FeMeasure
{
	...
	osgUtil::Intersector*
		CDPLineSegmentIntersector::clone(osgUtil::IntersectionVisitor& iv)
	{
		if (_coordinateFrame == MODEL && iv.getModelMatrix() == 0)
		{
			//GW: changed the next line
			osg::ref_ptr<CDPLineSegmentIntersector> lsi = new CDPLineSegmentIntersector(_start, _end);
			lsi->_parent = this;
		...
		//GW: changed the next line
		osg::ref_ptr<CDPLineSegmentIntersector> lsi = new CDPLineSegmentIntersector(_start * inverse, _end * inverse);
		lsi->_parent = this;
		lsi->_intersectionLimit = this->_intersectionLimit;
		return lsi.release();
	}
	...
	void
		CDPLineSegmentIntersector::intersect(osgUtil::IntersectionVisitor& iv, osg::Drawable* drawable)
	{
		if (reachedLimit()) return;

		osg::Vec3d s(_start), e(_end);
		if (!intersectAndClip(s, e, osgEarth::Utils::getBoundingBox(drawable))) return;
		...
		if (ti._hit)
		{
			//OSG_NOTICE << "Got one line intersections" << std::endl;
