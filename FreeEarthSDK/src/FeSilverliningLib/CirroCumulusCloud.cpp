...
CirroCumulusCloud::CirroCumulusCloud(CloudLayer *layer) : Cloud(layer), texture(0), vb(0), ib(0), fadeFalloff(5.0)
{

}

void CirroCumulusCloud::ApplyColor(const Color& c)
{
    if (c != appliedColor) {
        appliedColor = c;
...
void CirroCumulusCloud::Init(double w, double h)
{
    width = w;
    height = h;
    ...
    ren->LoadTextureFromFile("cirrocumulus.tga", &texture, false, true);
        ...
        if (verts) {
            ...
            for (int row = 0; row < gridDim; row++) {
                for (int col = 0; col < gridDim; col++) {
                    ...
                    verts[idx].SetColor(Color(1,1,1));
                }
            }
        }
...
void CirroCumulusCloud::ReloadShaders()
{
    Renderer *ren = Renderer::GetInstance();
    if (ren) {
...
bool CirroCumulusCloud::Cull(const Frustum& f)
{
...
void CirroCumulusCloud::DrawBlendedObject()
{
...
bool CirroCumulusCloud::Update(unsigned long now, bool forceUpdate)
{
    return false;
}

Vector3 CirroCumulusCloud::GetSortPosition() const
{
    double x, y, z;
...
double CirroCumulusCloud::GetDistance(const Vector3& from, const Vector3& to, const Matrix4& mv, bool rightHanded) const
{
