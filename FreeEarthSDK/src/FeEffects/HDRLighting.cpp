...
FeHDRLightingNode::FeHDRLightingNode(FeUtil::CRenderContext* rc,osg::Node* lightingModel,EGLARELIBTYPE lt,float starLengthScale,float starScale,float bloomScale)
	:m_rpModelNode(NULL)
...
FeHDRLightingNode::~FeHDRLightingNode()
{
	
	removeSceneCamFromRoot();
}

void FeHDRLightingNode::creatScene()
{
	createBloomTextures();
...
void FeHDRLightingNode::initUniform()
{
	if(m_rpLightingModel.valid())
	{
...
void FeHDRLightingNode::updateUniform()
{
	if(m_rpLightingModel.valid())
	{
...
void FeHDRLightingNode::updateLight()
{
	for (int i = 0;i < NUM_LIGHTS;i++)
	{
...
void FeHDRLightingNode::initSceneCamera()
{
...
void FeHDRLightingNode::insertSceneCamToRoot()
{
...
void FeHDRLightingNode::removeSceneCamFromRoot()
{
	if(m_opRenderContext.valid() && m_rpSceneCamera.valid())
	{
		osg::Group* root = m_opRenderContext->GetRoot();
...
void FeHDRLightingNode::preUpdate()
{
...
bool FeHDRLightingNode::checkSceneCamWithRoot()
{
...
void FeHDRLightingNode::initDownScale4x4Cam()
{
...
osg::Geode* FeHDRLightingNode::creatScreenQuadNode(float leftU,float rightU,float topV,float bottomV)
{
...
void FeHDRLightingNode::getTextureCoords( osg::ref_ptr<osg::Texture2D>& rpSrcTex,osg::Vec4d* srcRect,osg::ref_ptr<osg::Texture2D>& rpDestTex,osg::Vec4d* destRect,osg::Vec4d& outCoord )
{
	float tU, tV;
...
void FeHDRLightingNode::getSampleOffsets_DownScale4x4( int dwWidth, int dwHeight, osg::Vec2 avSampleOffsets[] )
{
...
void FeHDRLightingNode::measureLuminance()
{
	int i, x, y, index;
	osg::Vec2 avSampleOffsets[MAX_SAMPLES];
...
void FeHDRLightingNode::initAdaptation()
{
...
void FeHDRLightingNode::updateAdaptationTarget()
{
...
void FeHDRLightingNode::initSceneScaledToBrightPass()
{
...
void FeHDRLightingNode::initModelSceneCamera()
{
...
void FeHDRLightingNode::addChildToModelSceneCam()
{
	if(m_rpModelSceneCam.valid())
	{
...
void FeHDRLightingNode::initBrightPassToStarSource()
{
...
void FeHDRLightingNode::getSampleOffsetsGaussBlur5x5( unsigned int dwD3DTexWidth, unsigned int dwD3DTexHeight, osg::Vec2* avTexCoordOffset, osg::Vec4* avSampleWeights, float fMultiplier /*= 1.0f*/ )
{
...
float FeHDRLightingNode::gaussianDistribution( float x, float y, float rho )
{
...
void FeHDRLightingNode::initStarSourceToBloomSource()
{
...
void FeHDRLightingNode::getSampleOffsetsDownScale2x2( unsigned int dwWidth, unsigned int dwHeight, osg::Vec2 avSampleOffsets[] )
{
	if( NULL == avSampleOffsets )
		return;
...
void FeHDRLightingNode::initBloom()
{
	for (int i = 0;i < NUM_BLOOM_TEXTURES;i++)
	{
...
void FeHDRLightingNode::getSampleOffsetsBloom( unsigned int dwD3DTexSize, float afTexCoordOffset[15], osg::Vec4* avColorWeight, float fDeviation, float fMultiplier /*= 1.0f */ )
{
	int i = 0;
...
void FeHDRLightingNode::initStar()
{
	if(!m_texStarSource.valid())
		return;
...
void FeHDRLightingNode::initFinalSceneNode()
{
...
void FeHDRLightingNode::setLight( unsigned int index,const HDRLightParam& p )
{
	if(index < NUM_LIGHTS)
	{
...
HDRLightParam FeHDRLightingNode::getLight( unsigned int index )
{
	HDRLightParam p;
	if(index < NUM_LIGHTS)
	{
...
void FeHDRLightingNode::createBloomTextures()
{
	initSceneCamera();
...
void FeHDRLightingNode::deleteBloomTextures()
{
	removeSceneCamFromRoot();
	m_rpSceneCamera = NULL;
	m_rpDownScale4x4Cam = NULL;
...
void FeHDRLightingNode::updateBloomTextureSize()
{
	int texWidth = 0;
	int texHeight = 0;
	if(m_opRenderContext.valid())
...
void GeometryVisitor::apply( osg::Geometry& geometry )
{
}

GeometryVisitor::GeometryVisitor( osg::Node* n )
	:m_opModelNode(n)
{

}

void MyGeometryDrawCallback::drawImplementation( osg::RenderInfo& renderInfo,const osg::Drawable* drawable ) const
{
	if(drawable)
	{	
...
MyGeometryDrawCallback::MyGeometryDrawCallback( osg::Node* n )
	:m_opModelNode(n)
{

}

MyUpdateCallback::MyUpdateCallback( FeHDRLightingNode* ln )
...
void MyPreUpdateCallback::customOperator( const osgViewer::ViewerBase* viewerBase )
{
...
void MyStarCamGroupNodeCallback::operator()( osg::Node* node, osg::NodeVisitor* nv )
{
	if(m_opHDRLightingNode.valid())
	{
...
void MyStarCamDrawCallback::operator()( osg::RenderInfo& renderInfo ) const
{
	if(m_opHDRLightingNode.valid())
	{
...
void MyBloomCamDrawCallback::operator()( osg::RenderInfo& renderInfo ) const
{
	if(m_opHDRLightingNode.valid())
	{
		if(m_opHDRLightingNode->m_texStarSource.valid())
...
void MySceneCamDrawCallback::operator()( osg::RenderInfo& renderInfo ) const
{
...
void MyModelSceneCamDrawCallback::operator()( osg::RenderInfo& renderInfo ) const
{
	if(m_opHDRLightingNode.valid() && m_opHDRLightingNode->m_rpLightingModel.valid())
	{
		osg::Uniform* uRenderLight = m_opHDRLightingNode->m_rpLightingModel->getOrCreateStateSet()->getUniform("uRenderLight");
...
LightSphereGeomerty::LightSphereGeomerty(float r,int row /*= 10*/,int col /*= 10*/ )
	:m_row(row)
    ,m_col(col)
    ,m_radius(r)
{
...
void LightSphereGeomerty::CreateSphere()
{
...
void LightSphereGeomerty::SetColor( osg::Vec4 c )
{
	m_color = c;
