#ifndef _FE_GLAREDEF_H_
#define _FE_GLAREDEF_H_
...
#define SAFE_DELETE_ARRAY(p) { if (p) { delete[] (p);   (p)=NULL; } }
	...
	public:
		...
		static osg::Vec4    ms_avChromaticAberrationColor[8];
	...
	public:
		...
		bool             Initialize( const std::string szStarName,
			int nStarLines,
			int nPasses,
			float fSampleLength,
			float fAttenuation,
		...
		//  Sunny cross filter
		bool             Initialize_SunnyCrossFilter( const std::string szStarName = ( "SunnyCross" ),
			float fSampleLength = 1.0f,
			float fAttenuation = 0.88f,
			float fLongAttenuation = 0.95f,
			float fInclination = osg::DegreesToRadians( 0.0f ) );
	...
	public:
		...
		static const CStarDef& GetLib( unsigned int dwType )
		{
			return ms_pStarLib[dwType];
		}

		static const osg::Vec4& GetChromaticAberrationColor( unsigned int dwID )
		{
	...
	public:
		...
		static const CGlareDef& GetLib( unsigned int dwType )
		{
