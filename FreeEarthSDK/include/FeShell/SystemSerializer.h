...
namespace FeShell
{
    ...
    */
    ...
    public:
		...
		std::string GetPlotsConfig() const;
		void SetPlotsConfig(const std::string& strValue);
    ...
    private:
		...
		///解析到的系统名称
        std::string                m_strSystemTitle;		
		///解析系统的图标路
		std::string                m_strSystemIcon;			
		///矢量配置
		std::string                m_strFeatureLayerConfig; 
		///用户自定义图层配置
		std::string                m_strCustomLayerConfig;  
		///标记配置文件
		std::string                m_strMarksConfig;
		///军标配置文件
		std::string                m_strPlotsConfig;
		///眩晕图的默认配色方案
		std::string                m_strContourConfig;		
		///视点封装体
        FeUtil::CFreeViewPoint     m_sViewPointBegin;       
		///视点封装体
		FeUtil::CFreeViewPoint     m_sViewPointEnd;			
    };
}
