...
// 新增函数，为了支持网络矢量数据样式获取(g00066 2018.8.22)

// 根据 geometry 类型构建样式
bool MakeStyleByGeomerty( const Symbology::Geometry *geom, Style &style )
{
	// 如果样式中已经包含点线面，则不再查找
	if( style.has<LineSymbol>() && style.has<PointSymbol>() && style.has<PolygonSymbol>())
	{
		return false;
	}
	...
	case Geometry::TYPE_RING:
		style.getOrCreateSymbol<LineSymbol>();
		break;
	case Geometry::TYPE_POINTSET:
		style.getOrCreateSymbol<PointSymbol>();
		break;
	case Geometry::TYPE_POLYGON:
		style.getOrCreateSymbol<PolygonSymbol>();
		break;

	case Geometry::TYPE_MULTI:
		{
			const MultiGeometry* mulGeom = dynamic_cast<const MultiGeometry*>(geom);
			if(mulGeom)
			{
				const GeometryCollection& geomSet = mulGeom->getComponents();
				for( GeometryCollection::const_iterator conIter = geomSet.begin(); conIter != geomSet.end(); conIter++ )
				{
					if( !MakeStyleByGeomerty((*conIter).get(), style) )
					{
						break;
					}
				}
...
// 如果Style为空，则查找输入数据中所有Feature类型
Style GetStyleFromFeatures(const FeatureList &workingSet, FeatureModelGraph &fmg)
{
	for ( FeatureList::const_iterator conIter = workingSet.begin(); conIter != workingSet.end(); conIter++)
	{
		if( !MakeStyleByGeomerty( (*conIter)->getGeometry(), *(fmg.getStyles()->getDefaultStyle())) )
		{
			break;
		}
	}

	return *(fmg.getStyles()->getDefaultStyle());
}
// (g00066 2018.8.22)
                                    ...
                                    FeatureIndexBuilder* index)
{
    ...
    if ( cursor.valid() && cursor->hasMore() )
    {
		...
		// 如果Style为空，则查找输入数据中所有Feature类型 (g00066 2018.8.22 需要注意是否影响效率)
		if ( style.empty() && !workingSet.empty() )
		{
			Style styleFeature = GetStyleFromFeatures(workingSet, *this);

			if(styleFeature.has<LineSymbol>() && _session->getMapSRS() && !_session->getMapSRS()->isProjected())
			{
				styleFeature.getOrCreateSymbol<osgEarth::Symbology::AltitudeSymbol>(
					)->clamping() = osgEarth::AltitudeSymbol::CLAMP_TO_TERRAIN;
				styleFeature.getOrCreateSymbol<osgEarth::Symbology::AltitudeSymbol>(
					)->technique() = osgEarth::AltitudeSymbol::TECHNIQUE_GPU;
			}

			styleGroup = createStyleGroup(styleFeature, workingSet, context);
		}
		else
		{
