...
{
    // allocate pager threads based on CPU configuration.根据UPU配置来分配pager（寻呼机）线程
    ...
    // parse the command line.分析命令行
    osg::ArgumentParser arguments(&argc,argv);//获得提供命令行输入的参数
    osgViewer::Viewer viewer(arguments);//申请一个viewer，使用类对象arguments，作为参数，来输入main的输入参数

    // set up the motion model.建立运动模型
