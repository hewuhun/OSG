...
#define ELEM_TILETYPE "tiletype" //by y00001 增加type类型，用于更加flatgis灵活的判断
...
TileMap::createProfile() const
{
    ...
    {
		//y00001，默认的getGlobalGeodeticProfile修改为尖顶的了,TMS仍然构建平顶profile
		//return osgEarth::Registry::instance()->getGlobalGeodeticProfile();
		return osgEarth::Registry::instance()->getGlobalFlatGeodeticProfile();
    }
...
void
	TileMap::setTileType(std::string tileType)
{
	_tileType = tileType;
}

std::string TileMap::getTileType()
{
	return _tileType;
}
...
TileMapReaderWriter::read( const Config& conf )
{
	...
	//begin by y00001增加type，用来对flatgis更灵活的判断
	tileMap->setTileType      ( tileMapConf->value(ELEM_TILETYPE)); 
	//end
