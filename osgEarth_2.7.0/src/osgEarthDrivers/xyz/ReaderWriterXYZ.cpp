...
public:
	XYZSource(const TileSourceOptions& options) : 
	  ...
	  {
		  // 设置默认投影
		  if (_options.isBaidu().value())
		  {
			// 百度在线地图瓦片校准
			// 选取百度地图的部分区域作为数据源
			osgEarth::SpatialReference* srs = osgEarth::SpatialReference::create( "spherical-mercator" );
			const osgEarth::Profile* pf = osgEarth::Profile::create(
				srs, -16778766.00, -16777896, 16775666, 16776536+40000, 4, 4 );

			 this->setProfile(pf);
		  }
		  else
		  {
			  this->setProfile( osgEarth::Profile::createNamed("global-mercator") );
		  }
	  }
