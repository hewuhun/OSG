...
MPTerrainEngineNode::updateState()
{
    ...
    else
    {
        ...
        if ( Registry::capabilities().supportsGLSL() )
        {
			...
			// 二维中直接使用 osgEarth 默认 shader (by g00034)
			Shaders package;
			...
			package.replace( "$MP_TILECBUMP_UNIT", Stringify() << (_secondaryUnit+1) );
			...
			//BEGIN y00001 BUMP特性，设置第一个共享层坐标
			// Vertex shader:
			std::string vs = Stringify() <<
				...
				"    oe_layer_tilecbump = gl_MultiTexCoord" << _secondaryUnit+1 << ";\n"
				"}\n";
			//END y00001 BUMP特性，设置第一个共享层坐标
			...
			std::string fs = Stringify() <<
				...
				"uniform float oe_terrain_lightPow; \n"
				<< (useTerrainColor ?
			...
			//h00017
            terrainStateSet->addUniform(new osg::Uniform("oe_terrain_lightPow", _terrainOptions.terrainLightPow().get()));
#endif
