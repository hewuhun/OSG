...
MPGeometry::MPGeometry(const MPGeometry& rhs, const osg::CopyOp& cop) 
	...
	, _frame(rhs._frame)
	,_onlyDrawSurface(rhs._onlyDrawSurface)
{
	_layers.assign(rhs._layers.begin(), rhs._layers.end());
...
//h00017
void MPGeometry::accept(osg::PrimitiveFunctor& functor) const
{
...
bool MPGeometry::getOnlyDrawSurface()
{
	return _onlyDrawSurface;
}
void MPGeometry::setOnlyDrawSurface(bool flag)
{
	_onlyDrawSurface = flag;
	for(PrimitiveSetList::const_iterator itr = _primitives.begin();
		itr != _primitives.end();
		++itr)
	{
		if((*itr).valid())
		{
			osg::TileDrawElementsUShort* tes = dynamic_cast<osg::TileDrawElementsUShort*>((*itr).get());
			if(tes)
			{
				tes->_onlyDrawSurFace = flag;
			}
			else
			{
				osg::TileDrawElementsUInt* tei = dynamic_cast<osg::TileDrawElementsUInt*>((*itr).get());
				if(tei)
				{
					tei->_onlyDrawSurFace = flag;
				}
			}
		}
	}
