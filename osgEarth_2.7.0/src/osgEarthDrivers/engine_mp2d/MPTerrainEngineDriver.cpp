...
namespace osgEarth { namespace Drivers { namespace MPTerrainEngine
{
     ...
     */
    class MP2DTerrainEngineDriver : public osgDB::ReaderWriter
    {
    public:
        int _profiling;

        MP2DTerrainEngineDriver()
        {
            _profiling = 0;
            const char* p = ::getenv("OSGEARTH_MP2D_PROFILE");
            if ( p )
        ...
        {
            return "osgEarth MP2D Terrain Engine";
        }

        virtual bool acceptsExtension(const std::string& extension) const
        {
            return
                osgDB::equalCaseInsensitive( extension, "osgearth_engine_mp2d" ) ||
                osgDB::equalCaseInsensitive( extension, "osgearth_engine_mp_tile2d" ) ||
                osgDB::equalCaseInsensitive( extension, "osgearth_engine_mp_standalone_tile2d" );
        }

        virtual ReadResult readObject(const std::string& uri, const Options* options) const
        {
            if ( "osgearth_engine_mp2d" == osgDB::getFileExtension( uri ) )
            {
        ...
        virtual ReadResult readNode(const std::string& uri, const Options* options) const
        {
            ...
            if ( acceptsExtension(ext) )
            {
                ...
                if ( engineNode.valid() )
                {
                    ...
                    if ( "osgearth_engine_mp_tile2d" == ext )
                    {
                        node = engineNode->createNode(key, progress);
                    }
                    else if ( "osgearth_engine_mp_standalone_tile2d" == ext )
                    {
    ...
    REGISTER_OSGPLUGIN(osgearth_engine_mp2d, MP2DTerrainEngineDriver);
