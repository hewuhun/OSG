...
{    
    ...
    struct Data
    {
        ...
        osg::DrawElements* newDrawElements(GLenum mode) {
            ...
            if ( useUInt )
				...
				de = new osg::TileDrawElementsUInt(mode);
            else
                //de = new osg::DrawElementsUShort(mode);
				de = new osg::TileDrawElementsUShort(mode);
            de->setName("TMC");
            return de;
        }
		//h00017
    ...
    void createSkirtGeometry( Data& d, double skirtRatio )
    {
		...
		//h00017
		osg::TileDrawElementsUShort* tes = dynamic_cast<osg::TileDrawElementsUShort*>(elements.get());
		if(tes)
		{
			tes->_elementCountNeedToDraw = tes->size();
		}
		else
		{
			osg::TileDrawElementsUInt* tei = dynamic_cast<osg::TileDrawElementsUInt*>(elements.get());
			if(tei)
			{
				tei->_elementCountNeedToDraw = tei->size();
			}
		}
		//h00017
...
// 为图层对应的几何图形设置渲染数据（by g00034）
void SetRenderDataToLayerGeom(Data& d, const RenderLayer& renderLayer, MPGeometry& layerGeom)
{
 	layerGeom._layers.resize( 1 );

	if ( d.renderTileCoords.valid() )
		layerGeom._tileCoords = d.renderTileCoords;

	
	unsigned order = 0;//renderLayer._layer.getOrder();

	MPGeometry::Layer layer;
	layer._layerID        = renderLayer._layer.getUID();
	layer._imageLayer     = renderLayer._layer.getMapLayer();
	layer._tex            = renderLayer._layer.getTexture();
	layer._texParent      = renderLayer._layerParent.getTexture();

	// cache stock opacity. Disable if a color filter is installed, since
	// it can modify the alpha.
	layer._opaque =
		(renderLayer._layer.getMapLayer()->getColorFilters().size() == 0 ) &&
		(layer._tex.valid() && !renderLayer._layer.hasAlpha()) &&
		(!layer._texParent.valid() || !renderLayer._layerParent.hasAlpha()) &&
		(layer._imageLayer.valid() && layer._imageLayer->getMinVisibleRange() == 0.0f) &&
		(layer._imageLayer.valid() && layer._imageLayer->getMaxVisibleRange() == FLT_MAX);
	...
	// this for rendering because the scale/bias is already baked into the 
	// texture coordinates. BUT we still need it for sampling shared rasters etc.
	if ( renderLayer._layer._locator.valid() )
	{
		osg::Matrixd sbmatrix;

		renderLayer._layer._locator->createScaleBiasMatrix(
			d.model->_tileLocator->getDataExtent(),
			sbmatrix );
	...
	// parent texture matrix: it's a scale/bias matrix encoding the difference
	// between the two locators.
	if ( renderLayer._layerParent.getLocator() )
	{
		osg::Matrixd sbmatrix;

		renderLayer._layerParent.getLocator()->createScaleBiasMatrix(
			renderLayer._layer.getLocator()->getDataExtent(),
			sbmatrix );
	...
	// the texture coords:
	layer._texCoords  = renderLayer._texCoords.get();
	if ( renderLayer._texCoords.valid() )
	{
		int index = layerGeom.getTexCoordArrayList().size();
		layerGeom.setTexCoordArray( index, renderLayer._texCoords.get() );
	}

	layerGeom._layers[order] = layer;

	// the mask geometries:
	for ( MaskRecordVector::iterator mr = d.maskRecords.begin(); mr != d.maskRecords.end(); ++mr )
	{
		layer._texCoords = renderLayer._stitchTexCoords.get();
		mr->_geom->_layers[order] = layer;
		mr->_geom->_tileCoords = d.stitchTileCoords.get();
	}

	// install the tile coordinates in the geometry.
	if ( layerGeom._tileCoords.valid() )
	{
		int index = layerGeom.getTexCoordArrayList().size();
		layerGeom.setTexCoordArray( index, layerGeom._tileCoords.get() );
	}

	// elevation texture.
	layerGeom._elevTex = d.model->_elevationTexture.get();
}


/**
  * @class CMpGeomCullCallback
  * @brief 瓦片图元裁剪回调类，用于支持动态设置渲染顺序
  * @author g00034
*/
class CMpGeomCullCallback : public osg::Drawable::CullCallback
{
public:
	virtual bool cull(osg::NodeVisitor* nv, osg::Drawable* drawable, osg::RenderInfo* renderInfo) const 
	{
		MPGeometry* pgeom = dynamic_cast<MPGeometry*>(drawable);
		if(pgeom && pgeom->_layers.size() > 0)
		{
			pgeom->getOrCreateStateSet()->setRenderBinDetails(
				pgeom->_layers.begin()->_imageLayer->getRenderOrder(), "RenderBin");
                           ...
                           ProgressCallback* progress)
{
        ...
        d.surface->getVertexArray()->getNumElements() > 0 )
    {
		...
		if(d.renderLayers.size() > 0)
		{
			osg::ref_ptr<MPGeometry> rpGeomFreeLater = d.surface;
			d.surfaceGeode->removeDrawable(d.surface);

			/**
				分解图层，每个图层对应一个 MPGeometry
			*/
			int layerRenderOrder = 0;
			for( RenderLayerVector::const_iterator r = d.renderLayers.begin(); r != d.renderLayers.end(); ++r )
			{
				// 构造图层对应的 MPGeometry
				MPGeometry* pLayerGeom = (MPGeometry*)rpGeomFreeLater->clone(osg::CopyOp::DEEP_COPY_STATESETS);
				
				if( pLayerGeom )
				{
					SetRenderDataToLayerGeom(d, *r, *pLayerGeom);

					// 设置渲染顺序
					pLayerGeom->getOrCreateStateSet()->setRenderBinDetails(r->_layer.getMapLayer()->getRenderOrder(), "RenderBin");

					pLayerGeom->setCullCallback(new CMpGeomCullCallback());

					d.surfaceGeode->addDrawable( pLayerGeom );
				}
			}
		}
	}
