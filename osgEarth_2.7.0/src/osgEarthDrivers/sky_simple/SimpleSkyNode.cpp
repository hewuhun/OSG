...
//h00017
osg::Node* SimpleSkyNode::getAtmosphere()
{
	return _atmosphere.get();
}

osg::Node* SimpleSkyNode::getSun()
{
	return _sunXform.get();
}
osg::Node* SimpleSkyNode::getMoon()
{
	return _moonXform.get();
}
osg::Node* SimpleSkyNode::getStars()
{
	return _starsXform.get();
}
//h00017
...
SimpleSkyNode::makeAtmosphere(const osg::EllipsoidModel* em)
{
	...
	//h00017
	drawable->setEnableReflection(true);
	//h00017
	...
	//x00038
	m_ipUniformFogEnabled = new osg::Uniform("bFogEnabled", false);     // 雾化是否真正开启
	m_ipUniformFogColor   = new osg::Uniform("FogColor",    osg::Vec4f(0.66, 0.7, 0.81, 1.0));    // 雾化颜色
	atmosSet->addUniform(m_ipUniformFogEnabled);
	atmosSet->addUniform(m_ipUniformFogColor);
	//x00038 end
...
SimpleSkyNode::makeSun()
{
	...
	//h00017
	osg::Geometry* sg = s_makeDiscGeometry( sunRadius*80.0f );
	sg->setEnableReflection(true);
	sun->addDrawable(sg);
	//h00017
