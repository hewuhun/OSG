...
_options  ( options )
{
	...
	_osgFormat = _covFormat;

	_covFormat = "image/"+_covFormat;
	transform(_covFormat.begin(), _covFormat.end(), _covFormat.begin(), tolower);
}
                         ...
                         ProgressCallback*     progress)
{
	...
	std::string mimeTypeReturn = response.getMimeType();
	transform(mimeTypeReturn.begin(), mimeTypeReturn.end(), mimeTypeReturn.begin(), tolower);

	if( mimeTypeReturn.compare(_covFormat) != 0) return NULL;

    std::istream& input_stream = response.getPartStream( part_num );


    //TODO: un-hard-code TIFFs
    osgDB::ReaderWriter* reader = osgDB::Registry::instance()->getReaderWriterForExtension( _osgFormat );

    if ( !reader )
    {
        OE_NOTICE << "[osgEarth::WCS1.1] WARNING: no reader for " << _osgFormat << std::endl;
        return NULL;
    }
	...
	WCS11Source::createRequest( const TileKey& key ) const
{
	...
	req.addParameter( "crs", "EPSG:4326" );

	req.addParameter( "width", "256" );
	req.addParameter( "height", "256" );
