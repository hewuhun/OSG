...
public:
                             ...
                             NumericExpression& priorityExpr )
    {
		...
		PlaceNode* node = NULL;
		const osgEarth::Symbology::TextSymbol* textSymbol = style.get<osgEarth::Symbology::TextSymbol>();
		if (textSymbol)
		{
			osgEarth::Symbology::TextSymbol::Encoding encoding = textSymbol->encoding().get();
			if (encoding == osgEarth::Symbology::TextSymbol::ENCODING_UTF8)
			{
				std::string text = osgDB::convertStringFromCurrentCodePageToUTF8(textSymbol->content()->eval().c_str());
				node = new PlaceNode(0L, point, text, style);
			}
			else
			{
				node = new PlaceNode(0L, point, style, context.getDBOptions());
			}
		}
		else
		{
