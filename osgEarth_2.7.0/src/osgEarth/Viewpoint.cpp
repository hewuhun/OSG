...
Viewpoint::Viewpoint(const osg::Vec3d& focal_point, double heading_deg, double pitch_deg, double range, const osgEarth::SpatialReference* srs ) :
	//_point( focal_point ),
...
{
    //NOP
	_point = GeoPoint(srs, focal_point.x(), focal_point.y(), focal_point.z());
}
Viewpoint::Viewpoint(double x, double y, double z, double heading_deg, double pitch_deg, double range, const osgEarth::SpatialReference* srs ) :
	//_point( x, y, z ),
	_heading( heading_deg ),
	_pitch( pitch_deg ),
	_range( range ),
...
{
    //NOP
	_point = GeoPoint(srs, x, y, z);
}

Viewpoint::Viewpoint(const std::string& name, const osg::Vec3d& focal_point,
                     double heading_deg, double pitch_deg, double range, const osgEarth::SpatialReference* srs ) :
	_name( name ),
	//_point( focal_point ),
	_heading( heading_deg ),
	_pitch( pitch_deg ),
...
Viewpoint::Viewpoint(const std::string& name, double x, double y, double z,
                     double heading_deg, double pitch_deg, double range, const osgEarth::SpatialReference* srs ) :
	_name( name ),
	//_point( x, y, z ),
	_heading( heading_deg ),
...
Viewpoint::Viewpoint(double lon, double lat, double heading, double pitch, double range)
{
	_point->set( SpatialReference::get("wgs84"), lon, lat, 0.0, ALTMODE_ABSOLUTE );
	_heading->set( heading, Units::DEGREES );
	_pitch->set( pitch, Units::DEGREES );
...
//h00017
Viewpoint::Viewpoint(const char* name, double lon, double lat, double z, double h, double p, double range,const osgEarth::SpatialReference* srs)
{
	if (name) _name = name;
	_point->set( srs, lon, lat, z, ALTMODE_ABSOLUTE );
	_heading->set( h, Units::DEGREES );
	_pitch->set( p, Units::DEGREES );
	_range->set( range, Units::METERS );
}
...
const osgEarth::SpatialReference* Viewpoint::getSRS() const 
{
    return _srs.get();
}
