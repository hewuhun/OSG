...
//当前函数为兼容TMS
const Profile*
    Registry::getGlobalFlatGeodeticProfile() const
{
    if ( !_global_geodetic_flat_profile.valid() )
    {
        GDAL_SCOPED_LOCK;

        if ( !_global_geodetic_flat_profile.valid() ) // double-check pattern
        {

            const_cast<Registry*>(this)->_global_geodetic_flat_profile = Profile::create(
                "epsg:4326",
                -180.0, -90.0, 180.0, 90.0,
                "",
                2, 1 );
    ...
    return _global_geodetic_flat_profile.get();
}
