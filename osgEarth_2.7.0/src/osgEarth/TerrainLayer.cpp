...
TerrainLayerOptions::getConfig( bool isolate ) const
{
	...
	/*
	*c00005添加代码，读取earth文件中的缓存配置信息，从而根据每个图层配置的路径设置缓存
	*/
    conf.updateIfSet   ( "cachepath",    _cachePath );
    conf.updateIfSet   ( "cachedriver", _cacheDriver );
    conf.updateIfSet   ( "hastransparency", _hasTransparency );
...
TerrainLayerOptions::fromConfig( const Config& conf )
{
	...
	*c00005添加代码，读取earth文件中的缓存配置信息，从而根据每个图层配置的路径设置缓存
	*/
    conf.getIfSet   ( "cachepath", _cachePath );
    conf.getIfSet   ( "cachedriver", _cacheDriver );
    conf.getIfSet   ( "hastransparency", _hasTransparency );
...
TerrainLayer::setCache( Cache* cache )
{
    /*
	*c00005添加代码，初始化缓存
    *本函数首先判断该图层是否设置自己的缓存，如果设置了则使用自身的，
    *如果没有设置，则使用mapOptions的缓存配置项
	*/
    initCache();

    //if (_cache.get() != cache && getCachePolicy() != CachePolicy::NO_CACHE )
	if (!_cache.valid() && getCachePolicy() != CachePolicy::NO_CACHE )
    {
        _cache = cache;
        ...
        if ( newBin.valid() )
        {
            ...
            if ( meta.isValid() ) // cache exists and is valid.
            {
                ...
                if ( tileSource && getProfile() )
                {
					...
					//2015-5-29 c00005 cwf
					//判断缓存中的source_profile与当前数据的profile是否一致，如果不一致，
					//则说明缓存中的文件的格式与当前配置的路径的文件格式不同，所以设置为不使用以及不进行缓存数据
					//const Profile* cacheProfile = Profile::create(meta._sourceProfile.get());
					//if (!cacheProfile || !cacheProfile->isEquivalentTo(getTileSource()->getProfile()))
					//{
					//	OE_WARN << LC << "Cache has an incompatible profile... disabling"
					//		<< std::endl;
					//	setCachePolicy( CachePolicy::NO_CACHE );
					//	return 0L;
					//}
...
TerrainLayer::isCached(const TileKey& key) const
{
    ...
    /*
    *c00005修改代码，获取缓存策略首先从已经存在的缓存策略中获取
    */
	CacheBin * bin = const_cast<TerrainLayer*>(this)->getExistCacheBin(key.getProfile());
    if ( !bin )
        return false;
...
*c00005添加函数
*/
CacheBin * TerrainLayer::getExistCacheBin( const Profile* profile )
{
	if ( getCachePolicy() == CachePolicy::NO_CACHE )
	{
		return 0L;
	...
	if (_profile.valid())
	{
		if (_profile->isEquivalentTo(profile))
		{
...
// 重置缓存，缓存状态变更后调用(by g00034 为了解决动态更改缓存路径的问题)
void TerrainLayer::ResetCache()
{
	Threading::ScopedWriteLock exclusive(_cacheBinsMutex);
	_cacheBins.clear();
...
*c00005添加函数，初始化缓存
*/
void TerrainLayer::initCache()
{
	//如果_cache是无效的并且当前图层的缓存策略不为无缓存
	if ( !_cache.valid() && _runtimeOptions->cachePolicy().get() != CachePolicy::NO_CACHE )
	{
		if(!_runtimeOptions->cachePath().get().empty() || !_runtimeOptions->cacheDriver().get().empty() || !_runtimeOptions->cacheId().get().empty())
		{
			//如果cachePath没有设定，则使用earth文件所在的文件夹
			if (_runtimeOptions->cachePath().get().empty())
			{
				//cachePath使用earth文件所在的文件夹
				std::string earthPath = _runtimeOptions->referrer();

				//统一文件路径中的两种不同的路径符为"\"
				int posDoubleSlash = std::string::npos;
				while ((posDoubleSlash = earthPath.find("\\")) != std::string::npos)
				{
					earthPath.replace(posDoubleSlash, 1, "/");
				} 
				int pos = earthPath.find_last_of("/");

				_runtimeOptions->cachePath().init(earthPath.substr(0, pos).append("/FileCache"));

				//如果cacheId没有被设置，这时cacheId使用图层名称+“Cache“
				if (_runtimeOptions->cacheId().get().empty())
				{
					std::string layerName = _runtimeOptions->name();
					_runtimeOptions->cacheId().init( layerName.append("Cache"));
				}
			}
			//否则cachePath已经被设置过
			else
			{
				std::string cachePath = _runtimeOptions->cachePath().get();
				//如果cacheId没有被设置，这时cacheId使用设置的cachePath的最后一个文件夹
				if (_runtimeOptions->cacheId().get().empty())
				{   
					//统一文件路径中的两种不同的路径符为"\"
					int posDoubleSlash = std::string::npos;
					while ((posDoubleSlash = cachePath.find("\\")) != std::string::npos)
					{
						cachePath.replace(posDoubleSlash, 1, "/");
					} 
					int pos = cachePath.find_last_of("/");
					std::string cacheId = cachePath.substr(pos+1, cachePath.size());

					_runtimeOptions->cachePath().init(cachePath.substr(0, pos));

					//如果设置的cachePath只是一个盘符的路径，则cacheId使用图层名称+“Cache“
					if (cacheId.empty())
					{
						std::string layerName = _runtimeOptions->name();
						_runtimeOptions->cacheId().init(layerName.append("Cache"));
					}
					//否则使用cachePath的最后一个文件夹
					else
					{
						_runtimeOptions->cacheId().init(cacheId);
					}
				}
			}

			//如果读写缓存的插件没有被定义，则默认使用filesystem插件
			if (_runtimeOptions->cacheDriver().get().empty())
			{
				_runtimeOptions->cacheDriver().init("filesystem");
			}

			//配置缓存的配置项并创建缓存,首先如果是相对路径，则转化为绝对路径
			std::string strPath = URI( _runtimeOptions->cachePath().get(), _runtimeOptions->referrer() ).full();

			Config conf("cache");
			conf.add("driver", _runtimeOptions->cacheDriver().get());
			conf.add("path", strPath);

			CacheOptions cacheOption(conf);

			_cache = CacheFactory::create(cacheOption);
		}
	}
}
