...
TerrainEngineNode::traverse( osg::NodeVisitor& nv )
{
    if ( nv.getVisitorType() == osg::NodeVisitor::CULL_VISITOR )
    {
		...
		if(cv)
		{
			unsigned int cfn = cv->getFrameStamp()->getFrameNumber();
			if(_lastCollectTileFrameNumber != cfn)
			{
				_lastCollectTileFrameNumber = cfn;

				if(cv->getReflecting())
					return;

				cv->setNeedCollectTileNode(true);
				cv->getTileNodeList().clear();

				osg::CoordinateSystemNode::traverse( nv );

				cv->setNeedCollectTileNode(false);
				return;
			}	
		}
		//h00017
