...
namespace
{
	...
	// w00024 start
	void removeQuotedPragmaString(std::string& source,
		const std::string& key)
	{
		std::string::size_type includePos = source.find("#pragma " + key);
		if ( includePos == std::string::npos )
		...
		osgEarth::replaceIn(source, statement, "");
                   ...
                   const osgDB::Options* dbOptions)
{
    ...
    if ( !entryPoint.empty() )
    {
		...
		// w00024 start
		removeQuotedPragmaString(source, "vp_location");
		removeQuotedPragmaString(source, "vp_entryPoint");
		removeQuotedPragmaString(source, "vp_order");
		// w00024 end	
