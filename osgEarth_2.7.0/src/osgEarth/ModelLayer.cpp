...
namespace osgEarth
{
	class CElevationLODEx : public osg::Group
	{
	private:
	...
	public:
		META_Node(osgEarth, CElevationLODEx);
		...
		CElevationLODEx( const CElevationLODEx& rhs, const osg::CopyOp& op) :
		...
		CElevationLODEx(const SpatialReference* srs):
		_minElevation( -DBL_MAX ),
			_maxElevation( DBL_MAX ),
			_minRange    ( 0.0f ),
			_maxRange    ( FLT_MAX ),
		...
		CElevationLODEx(const SpatialReference* srs, double minElevation, double maxElevation):
		_minRange    ( 0.0f ),
			_maxRange    ( FLT_MAX ),
			_srs         ( srs ),
			m_bChildVisible(true)
		...
		void traverse( osg::NodeVisitor& nv)
		{
				...
				nv.getTraversalMode() == osg::NodeVisitor::TRAVERSE_ACTIVE_CHILDREN )
			{
				...
				if ( rangeOK )
				{
					...
					if(m_bChildVisible != altitudeOK)
					{
						m_bChildVisible = altitudeOK;
						...
						for(int n = 0; n < numChild; ++n)
						{
							if(getChild(n))
							{
								getChild(n)->setNodeMask( altitudeOK ? ~0 : 0);
							}
						}
					}
...
void 
ModelLayer::setVisibleRange(double dMin, double dMax)
{
	_minVisible = dMin;
	_maxVisible = dMax;
	...
	for(Graphs::iterator i = _graphs.begin(); i != _graphs.end(); ++i)
	{
		if ( i->second.valid() )
		{
			CElevationLODEx* rangLod = dynamic_cast<CElevationLODEx*>(i->second.get());
			if(rangLod)
			{
				rangLod->setElevations(_minVisible, _maxVisible);
			}
		}
	}
}
void 
ModelLayer::getVisibleRange(double &dMin, double &dMax)
{
	dMin = _minVisible ;
	dMax = _maxVisible ;
}
                                  ...
                                  ProgressCallback* progress )
{
    ...
    if ( _modelSource.valid() )
    {
        ...
        if ( node )
        {
			...
			// add by g00066 支持矢量图层可视范围设置
			CElevationLODEx* rangLod = new CElevationLODEx(map->getSRS());
			rangLod->setElevations(_minVisible, _maxVisible);
			rangLod->addChild(node);
			node = rangLod;
			// add by g00066
...
void ModelLayer::setRenderOrder(int order)
{
	Layer::setRenderOrder(order);

	for(Graphs::iterator i = _graphs.begin(); i != _graphs.end(); ++i)
	{
		if ( i->second.valid() )
			i->second->getOrCreateStateSet()->setRenderBinDetails(order, "RenderBin");
	}
}