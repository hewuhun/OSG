...
// 				if(cullStack)
// 				{
// 					osg::Vec3d camPos = cullStack->getViewPointLocal();
// 					osg::Vec3d camDir = camPos - getCenter();
// 					camDir.normalize();
// 					osg::Vec3d centerDir = getCenter();
// 					centerDir.normalize();
// 
// 					double dotLC = centerDir * camDir;
// 					dotLC = osg::clampBetween(dotLC,-1.0,1.0);
// 					required_angle = abs(acos(dotLC));
// 
// 					if(required_angle > osg::PI / 3.0)
// 						required_angle = osg::PI / 3.0;
// 
// 					required_range = required_range / cos(required_angle);
// 				}
            }
            else
            {
            ...
            if (needToLoadChild)
            {
                    ...
                    numChildren<_perRangeDataList.size())
                {
                    ...
                    else
                    {
						...
						//x00038 begin 2018/10/18
						//倾斜摄影 网络数据 子节点的路径中 + 替换为 %2B
						if (0 == _databasePath.substr(0, 7).compare("http://"))
						{
							std::string strChildPath;
							for (int i=0; i<_perRangeDataList[numChildren]._filename.length(); i++)
							{
								if ('+' !=  _perRangeDataList[numChildren]._filename.at(i))
								{
									strChildPath +=  _perRangeDataList[numChildren]._filename.at(i);
								}
								else
								{
									strChildPath += "%2B";
							 ...
							 nv.getDatabaseRequestHandler()->requestNodeFile(_databasePath+strChildPath,nv.getNodePath(),priority,nv.getFrameStamp(), _perRangeDataList[numChildren]._databaseRequest, _databaseOptions.get());
						}
                        else
						{
							//源代码
