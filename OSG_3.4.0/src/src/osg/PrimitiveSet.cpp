...
void osg::TileDrawElementsUInt::draw( osg::State& state, bool useVertexBufferObjects ) const
{
	GLenum mode = _mode;
#if defined(OSG_GLES1_AVAILABLE) || defined(OSG_GLES2_AVAILABLE)
	if (mode==GL_POLYGON) mode = GL_TRIANGLE_FAN;
	...
	if(_onlyDrawSurFace)
		dcount = _elementCountNeedToDraw;
	...
	if (useVertexBufferObjects)
	{
		...
		if (ebo)
		{
			if (_numInstances>=1) state.glDrawElementsInstanced(mode, dcount, GL_UNSIGNED_INT, (const GLvoid *)(ebo->getOffset(getBufferIndex())), _numInstances);
			else glDrawElements(mode, dcount, GL_UNSIGNED_INT, (const GLvoid *)(ebo->getOffset(getBufferIndex())));
		}
		else
		{
			if (_numInstances>=1) state.glDrawElementsInstanced(mode, dcount, GL_UNSIGNED_INT, &front(), _numInstances);
			else glDrawElements(mode, dcount, GL_UNSIGNED_INT, &front());
		}
	}
	else
	{
...
void osg::TileDrawElementsUShort::draw( osg::State& state, bool useVertexBufferObjects ) const
{
	GLenum mode = _mode;
#if defined(OSG_GLES1_AVAILABLE) || defined(OSG_GLES2_AVAILABLE)
	if (mode==GL_POLYGON) mode = GL_TRIANGLE_FAN;
	...
	if (useVertexBufferObjects)
	{
		...
		if (ebo)
		{
			if (_numInstances>=1) state.glDrawElementsInstanced(mode, dcount, GL_UNSIGNED_SHORT, (const GLvoid *)(ebo->getOffset(getBufferIndex())), _numInstances);
			else glDrawElements(mode, dcount, GL_UNSIGNED_SHORT, (const GLvoid *)(ebo->getOffset(getBufferIndex())));
		}
		else
		{
			if (_numInstances>=1) state.glDrawElementsInstanced(mode, dcount, GL_UNSIGNED_SHORT, &front(), _numInstances);
			else glDrawElements(mode, dcount, GL_UNSIGNED_SHORT, &front());
		}
	}
	else
	{
