...
// 描述：初始化
//---------------------------------------------------------------------
void DriveManipulator::GetHPRFromQuat(const osg::Quat& q, double &h, double &p, double &r)
{
	osg::Matrixd rot(q);
	p = asin(rot(1,2));
...
// 描述：旋转角度，单位为Degree
//---------------------------------------------------------------------
double DriveManipulator::GetHeading()
{
	osg::Quat qt = getRotation();

	double h,p,r;
	GetHPRFromQuat(qt,h,p,r);
	//旋转角
	double dHeading = RadiansToDegrees(r)-180;
	return dHeading;
}
...
// 描述：俯仰角度，单位为Degree
//---------------------------------------------------------------------
double DriveManipulator::GetPitch()
{ 
	return RadiansToDegrees(_pitch);
}
//---------------------------------------------------------------------
// 函数：GetPitch()
// 描述：俯仰角度，单位为Degree
//---------------------------------------------------------------------
void DriveManipulator::SetPitch(double in_p)
{
	_pitch = DegreesToRadians(in_p);
}
