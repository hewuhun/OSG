...
void Viewer::updateTraversal()
{
	...
	if(!_preUpdateCallbackList.empty())
	{
		for (UpdateCallbackList::iterator it = _preUpdateCallbackList.begin(); it != _preUpdateCallbackList.end(); it++)
		{
			if((*it).valid())
			{
				(*it)->customOperator(this);
			}
		}
	}
	//h00017
    ...
    //h00017
	if(!_postUpdateCallbackList.empty())
	{
		for (UpdateCallbackList::iterator it = _postUpdateCallbackList.begin(); it != _postUpdateCallbackList.end(); it++)
		{
			if((*it).valid())
			{
				(*it)->customOperator(this);
