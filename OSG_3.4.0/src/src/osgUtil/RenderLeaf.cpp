...
void RenderLeaf::render(osg::RenderInfo& renderInfo,RenderLeaf* previous)
{
    ...
    {
		//h00017
		updateShadowMapWorldViewProjMatUniform(renderInfo);
		//h00017
		state.applyWorldMatrix(_worldMat.get());
		//h00017
...
void osgUtil::RenderLeaf::updateShadowMapWorldViewProjMatUniform( osg::RenderInfo& renderInfo )
{
	osg::Camera* cam = renderInfo.getCurrentCamera();
	if(cam && _drawable.valid())
	{
		osg::View* vv = cam->getView();
		if(vv)
		{
			osg::Matrix mat[3];

			osg::Uniform* u = _drawable->getShadowMapWorldViewProjMatUniform();
			if(u && _worldMat.valid())
			{
				for (int i = 0;i < 3;i++)
				{
					u->setElement(i,(*_worldMat) * vv->getShadowMapViewProjMat(i));
				}
			}
		}
	}
