...
CullVisitor::CullVisitor():
	...
	//h00017
	_needCollectTileNode(false),
	_collectTileNodeCallback(NULL),
	_isReflecting(false)
	//h00017
{
...
void CullVisitor::apply(osg::Drawable& drawable)
{
    ...
    else
    {
		...
		if(_isReflecting)
		{
			if(_reflectionCallback.valid())
			{
				bool flag = _reflectionCallback->checkNeedReflection(drawable,this);
				if(flag)
				{
            ...
            addDrawableAndDepth(&drawable,&matrix,depth);
		if(_needCollectTileNode)
		{
			if(_collectTileNodeCallback.valid())
			{
				_collectTileNodeCallback->collectTileNodeImplementation(drawable,this);
			}
		}
		//h00017
    }
...
*/
        ...
        else
        {
			...
			if(_isReflecting)
			{
				if(_reflectionCallback.valid() && drawable)
				{
					bool flag = _reflectionCallback->checkNeedReflection(*drawable,this);
					if(flag)
					{
...
void CullVisitor::apply(Transform& node)
{
	...
	//h00017
	//bool reflecting = _isReflecting;
	//if(reflecting)
	{
		RefMatrix* worldMat = createOrReuseMatrix(*getWorldMatrix());
		node.computeLocalToWorldMatrix(*worldMat,this);
		pushWorldMatrix(worldMat);
	}
	//h00017
